...
class ExtraParser extends AbstractParser {
	...
	private function setJsonData() {
		...
		$json = $this->getJson($this->content, 'type="application/json">', '</script>');
		if ($json) {
			$json = iconv('UTF-8', 'UTF-8//IGNORE', $json);
	...
	public function getImages() {
		...
		if (isset($this->jsonDataArray['props']['initialState']['Product']['product']['skus'])
			...
			foreach ($this->jsonDataArray['props']['initialState']['Product']['product']['skus'] as $key => $imgsLink) {
				$linksId = $imgsLink['id'];
				$skuLink = 'https://pdp-api.extra.com.br/api/v2/sku/source/EX?skuId=' . $linksId;
	...
	public function getCustomerReviews( $maxReviews = 0, &$reviews = array(), $page = 1) {
		if (!$reviews) {
			...
			$this->reviewLink = 'https://pdp-api.extra.com.br/api/v2/reviews/product/' . $sku . '/source/EX?size=20&orderBy=DATE';
		}
